 ...
 * <a href="http://medialab.di.unipi.it/wiki/Tanl_POS_Tagset">TANL tagset</a>, a conversion process
 * is needed before calling MorphIT.
 * <p/>
 * This implementation therefore uses a resources file, called "tanl-morphit" to achieve the
...
public class MorphITLemmatizer extends ISingleablePlugin<Message, VoidConfig> {
    ...
    private HashMap<String, HashSet<String>> getTanlMorphITMap() {
        if (tanlMorphITMap == null) {
            InputStream mapStream =
                    MorphITLemmatizer.class.getClassLoader().getResourceAsStream("tanl-morphit");
     ...
     * <ul>
     * <li>convert the TANL POS tag on the {@link Token} to a {@link Set} of possible MorphIT
     * tags</li>
     * <li>get a list of possible lemmas for the word</li>
     * <li>for each candidate lemma and for each MorphIT tag of the token, the lemma is returned if
     * the tag is compatible with the one from the candidate lemma</li>
     * </ul>
     * <p/>
     * If the {@link Token} is considered a stop word the lemmatization process doesn't start at
     * all.
     *
